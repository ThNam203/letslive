...
export default function SearchBar({
    ...
    return (
        <div className={cn("relative w-full", className)}>
            ...
            {isLoading && query && (
                <div className="rounded-xs bg-background absolute mt-1 w-full border p-4 shadow-md">
            ...
            {showResults && results.length > 0 && !isLoading && (
                <div className="rounded-xs bg-background absolute z-10 mt-1 max-h-60 w-full overflow-auto shadow-md">
                    {results.map((user) => (
                        <Link
                            key={user.id}
                            href={`/users/${user.id}`}
