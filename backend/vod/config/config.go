...
func (cm *ConfigManager) fetchAndParseConfig() (*Config, error) {
	...
	logger.Infof(cm.ctx, "fetching config from: %s", url)
	...
	dbUser := os.Getenv("VOD_DB_USER")
	dbPassword := os.Getenv("VOD_DB_PASSWORD")
	if dbUser == "" || dbPassword == "" {
		logger.Warnf(cm.ctx, "database credentials (VOD_DB_USER, VOD_DB_PASSWORD) not found in environment.")
	}
