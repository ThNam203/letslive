...
func (h *NotificationHandler) MarkAsReadPrivateHandler(w http.ResponseWriter, r *http.Request) {
	...
	ctx, span := tracer.MyTracer.Start(ctx, "notification_handler.mark_as_read")
	serviceErr := h.notificationService.MarkAsRead(ctx, notificationId, userId.String())
	span.End()
