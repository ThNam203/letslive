...
func TestFromErrorSeesThroughWrapping(t *testing.T) {
	got := FromError(fmt.Errorf("getuserbyid: %w", domains.ErrUserNotFound))
	if got.Key != RES_ERR_USER_NOT_FOUND_KEY {
		t.Errorf("key = %q, want %q", got.Key, RES_ERR_USER_NOT_FOUND_KEY)
	}
}
...
// An unrecognised error must not leak its text to the client. The user
// repositories inspect pgconn codes directly, so a raw driver error reaching
// the handler is the realistic failure here.
func TestFromErrorUnknownIsInternal(t *testing.T) {
	got := FromError(errors.New(`ERROR: duplicate key value violates unique constraint "users_username_key" (SQLSTATE 23505)`))
