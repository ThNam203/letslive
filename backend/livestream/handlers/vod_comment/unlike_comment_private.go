...
func (h *VODCommentHandler) UnlikeCommentPrivateHandler(w http.ResponseWriter, r *http.Request) {
	...
	ctx, span := tracer.MyTracer.Start(ctx, "unlike_vod_comment_private_handler")
	serviceErr := h.commentService.UnlikeComment(ctx, commentId, *userUUID)
	span.End()
