...
func (h *VODCommentHandler) DeleteCommentPrivateHandler(w http.ResponseWriter, r *http.Request) {
	...
	ctx, span := tracer.MyTracer.Start(ctx, "delete_vod_comment_private_handler")
	serviceErr := h.commentService.DeleteComment(ctx, commentId, *userUUID)
	span.End()
