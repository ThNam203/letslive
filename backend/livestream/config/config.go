...
func RetrieveConfig(registry discovery.Registry) *Config {
	...
	config.Database.ConnectionString = fmt.Sprintf("postgres://%s:%s@%s:%d/%s?%s", os.Getenv("LIVESTREAM_DB_USER"), os.Getenv("LIVESTREAM_DB_PASSWORD"), config.Database.Host, config.Database.Port, config.Database.Name, strings.Join(config.Database.Params, "&"))
