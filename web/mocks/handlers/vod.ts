...
import { vods, vodComments, likedCommentIds, ME_USER_ID, uid, now } from "../db";
...
export const vodHandlers = [
    ...
    http.get(`${API_BASE}/vods`, ({ request }) => {
        ...
        return ok<VOD[]>(slice, { page, page_size: limit, total: results.length });
    }),
    ...
    http.get(`${API_BASE}/popular-vods`, ({ request }) => {
        ...
        return ok<VOD[]>(slice, { page, page_size: limit, total: sorted.length });
    }),
    ...
    http.delete(`${API_BASE}/vods/:vodId`, ({ params }) => {
        ...
        if (idx === -1) return notFound("res_err_vod_not_found", "VOD not found");
        vods.splice(idx, 1);
        return noContent();
    }),
    ...
    http.post(`${API_BASE}/vods/upload`, async ({ request }) => {
        ...
        const newVod: VOD = {
            ...
            visibility: ((formData.get("visibility") as string) ?? "public") as VOD["visibility"],
            thumbnailUrl: null,
            viewCount: 0,
            duration: 0,
    ...
    http.get(`${API_BASE}/vods/:vodId/comments`, ({ request, params }) => {
        ...
        return ok<VODComment[]>(slice, { page, page_size: limit, total: topLevel.length });
    }),

    // GET /vod-comments/:commentId/replies
    http.get(`${API_BASE}/vod-comments/:commentId/replies`, ({ request, params }) => {
        ...
        return ok<VODComment[]>(slice, { page, page_size: limit, total: replies.length });
    }),

    // POST /vods/:vodId/comments
    http.post(`${API_BASE}/vods/:vodId/comments`, async ({ params, request }) => {
        const { vodId } = params as { vodId: string };
    ...
    http.delete(`${API_BASE}/vod-comments/:commentId`, ({ params }) => {
        ...
        if (!comment) return notFound("res_err_vod_comment_not_found", "Comment not found");
    ...
    http.post(`${API_BASE}/vod-comments/liked-ids`, async ({ request }) => {
        ...
        const ids = (body?.commentIds ?? []).filter((id) => likedCommentIds.has(id));
